...
export class TypeCTypeProvider {
    ...
    getIdentifiableFields(type: TypeDescription): AstNode[] {
        ...
        if (isNamespaceType(type)) {
            // Unwrap VariableDeclarationStatement to individual VariableDeclaration nodes
            // (VariableDeclarationStatement has no name, but its children do)
            const results: AstNode[] = [];
            for (const def of type.declaration.definitions) {
                if (ast.isVariableDeclarationStatement(def)) {
                    results.push(...def.declarations.variables);
                } else {
                    results.push(def);
                }
            }
