...
import { createCoroutineType, isDescCoroutineType } from "./datatypes/coroutine-type.js";
import {
...
import { getArrayPrototypeReturnType, getCoroutinePrototypeReturnType } from "./builtin-type-utils.js";

export type NamedTypedAstNode = AstNode & { name: string, typeDescription?: TypeDescription };
...
export class TypeCTypeProvider {
    ...
    inferFunctionCall(node: ast.FunctionCall): TypeDescription {
        ...
        if (isDescFunctionType(fnType)) {
            ...
            // get the parent base Expr:
            const baseExpr = (node.expr as ast.MemberAccess).expr;
            const baseExprType = this.inferExpression(baseExpr);
            if (isDescArrayType(baseExprType)) {
                return getArrayPrototypeReturnType(fnType, baseExprType);
            }
            else if (isDescCoroutineType(baseExprType)) {
                return getCoroutinePrototypeReturnType(fnType, baseExprType);
            }

            return fnType.returnType ?? createVoidType();
        }
    ...
    inferMemberAccess(node: ast.MemberAccess): TypeDescription {
        ...
        return field.typeDescription ?? this.inferType(field);
    }
    ...
    getIdentifiableFields(type: TypeDescription): NamedTypedAstNode[] {
          ...
          */
            return type.cases
                ...
                .map((c) => ({
                    ...c.$node!,
                    name: c.name,
                    typeDescription: type, // Enum values are of type enum itself
                }));
        ...
        //else if(isDescVariantType(type))
            ...
            return type.variant.constructors.map((c) => ({
                ...c.$node!,
                name: c.name,
                typeDescription: c, // Variant constructors are of type variant constructor itself
            }));
    ...
    getClassIdentifiableFields(type: ClassTypeDescription): NamedTypedAstNode[] {
        console.log(type);
    ...
    getClassStaticIdentifiableFields(type: ClassTypeDescription): NamedTypedAstNode[] {
        // TODO: merge class with its supertypes & implementation methods
        const res = [
            ...type.attributes
    ...
    getNamespaceIdentifiableFields(namespace: ast.NamespaceDecl): NamedTypedAstNode[] {
        const exportedNodes: NamedTypedAstNode[] = [];
    ...
    getFFIIdentifiableFields(ffi: ast.ExternFFIDecl): NamedTypedAstNode[] {
        return ffi.methods;
    }

    getArrayIdentifiableFields(type: ArrayTypeDescription): NamedTypedAstNode[] {
        /**
        ...
        const allElements = this.indexManager
            ...
            .flat().map(e => ({...e, typeDescription: this.inferType(e)}));
        return allElements;
    }
}
