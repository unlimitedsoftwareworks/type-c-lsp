...
import {
    areTypesEqual as areTypesEqualStandalone,
    areStructTypesEqual as areStructTypesEqualStandalone,
    areFunctionTypesEqual as areFunctionTypesEqualStandalone,
    areReferenceTypesEqual as areReferenceTypesEqualStandalone,
} from "./type-equality.js";
import {
    substituteGenerics as substituteGenericsStandalone,
    type SubstitutionDeps,
...
export class TypeCTypeUtils {
    ...
    /** Dependencies for the standalone substitution engine. */
    private readonly substitutionDeps: SubstitutionDeps;
    /** Mutable depth tracking state for the substitution engine. */
    private readonly substitutionState: SubstitutionState;
    ...
    constructor(services: TypeCServices) {
        ...
        // Initialize substitution engine dependencies (bound lazily where needed)
        this.substitutionDeps = {
            typeFactory: this.typeFactory,
            isTypeBasic: (type) => this.isTypeBasic(type),
            areTypesEqual: (a, b) => this.areTypesEqual(a, b),
            resolveReference: (type) => this.typeProvider().resolveReference(type),
            resolvingReferences: this.resolvingReferences,
            pendingChecks: this.pendingChecks,
            addPendingCheck: (from, to) => this.addPendingCheck(from, to),
            removePendingCheck: (from, to) => this.removePendingCheck(from, to),
        };
        this.substitutionState = { depth: 0, maxDepth: 50 };
    }
    ...
    // ============================================================================
    // Type Equality (delegates to standalone functions in type-equality.ts)
    // ============================================================================

    areTypesEqual(a: TypeDescription, b: TypeDescription): TypeCheckResult {
        return areTypesEqualStandalone(a, b, (t) => this.resolveIfReference(t));
    }

    areStructTypesEqual(a: StructTypeDescription, b: StructTypeDescription): TypeCheckResult {
        return areStructTypesEqualStandalone(a, b, (t) => this.resolveIfReference(t));
    }

    areFunctionTypesEqual(a: FunctionTypeDescription, b: FunctionTypeDescription): TypeCheckResult {
        return areFunctionTypesEqualStandalone(a, b, (t) => this.resolveIfReference(t));
    }

    areReferenceTypesEqual(a: ReferenceTypeDescription, b: ReferenceTypeDescription): TypeCheckResult {
        return areReferenceTypesEqualStandalone(a, b, (t) => this.resolveIfReference(t));
    }
    ...
    // ============================================================================
    // Type Substitution (delegates to standalone functions in type-substitution.ts)
    // ============================================================================
    ...
    substituteGenerics(
        ...
        return substituteGenericsStandalone(type, substitutions, this.substitutionDeps, this.substitutionState, context, errors);
    }
